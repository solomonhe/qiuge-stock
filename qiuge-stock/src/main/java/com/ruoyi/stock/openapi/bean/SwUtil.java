package com.ruoyi.stock.openapi.bean;
...
public class SwUtil {
     ...
     * @return
     */
    public synchronized List<SwTradeInfoEntity> getDaySwTradeTwo(int page,long pageCount) {
     ...
     * @return
     */
    public synchronized List<SwTradeInfoEntity> getAllSwTradeTwo(int page,long pageCount) {
        String url = this.allUrl(page, pageCount);
        return this.send(url);
    }

    private List<SwTradeInfoEntity> send(String url){
        String post = this.get(url);
        Object root = JSONObject.parseObject(post).get("root");
        return JSONArray.parseArray(JSONObject.toJSONString(root), SwTradeInfoEntity.class);
    }
