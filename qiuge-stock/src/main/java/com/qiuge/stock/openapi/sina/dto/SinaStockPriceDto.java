...
public class SinaStockPriceDto {
    ...
    @JsonIgnore
    public static List<StockTradeDayAverage> convert(List<SinaStockPriceDto> stockPriceDtoList, Long tradeDate) {

        log.info("stockPriceDtoList size :{}", stockPriceDtoList.size());
        List<StockTradeDayAverage> averageEntityList = new ArrayList<>(stockPriceDtoList.size());
        stockPriceDtoList.stream().forEach(sinaStockPriceDto -> {
            StockTradeDayAverage stockTradeDayInfo = new StockTradeDayAverage();
            ...
            stockTradeDayInfo.setStockAmount(sinaStockPriceDto.getAmount());
